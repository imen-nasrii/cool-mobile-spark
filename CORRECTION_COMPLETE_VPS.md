# Correction Complète - Fichiers VPS

Le fichier objectStorage.ts n'est pas complet. Voici les commandes pour corriger :

## 1. Recréer le fichier objectStorage.ts complet

```bash
rm server/objectStorage.ts
cat > server/objectStorage.ts << 'ENDFILE'
...
## 2. Ajouter les routes multer dans routes.ts

```bash
# Trouver la ligne avec les routes object storage et ajouter les nouvelles routes
sed -i '/app.post.*\/api\/objects\/upload.*authenticateToken/a\
\
...
Exécutez ces 3 étapes dans l'ordre.