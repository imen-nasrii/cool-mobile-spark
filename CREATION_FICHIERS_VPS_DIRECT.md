# Création Directe des Fichiers sur VPS

## ÉTAPE 1: Remplacer server/objectStorage.ts

```bash
# Créer le nouveau fichier objectStorage.ts
...
## ÉTAPE 2: Ajouter les routes multer dans server/routes.ts

```bash
# Ajouter la route multer après les autres routes object storage
sed -i '/\/\/ Object storage routes for profile photos/a\
...
```bash
# Build avec les nouveaux fichiers
npm run build

# Redémarrer l'application
pm2 restart tomati-hamdi

# Vérifier les logs (plus d'erreur "Object storage not available")
pm2 logs tomati-hamdi --lines 10
```

**Exécutez ces commandes dans l'ordre sur votre VPS.**